...
public class DecentralizedReference<T> extends RegistryEntry.Reference<T> {
    ...
    public DecentralizedReference(@NotNull Reference<T> reference, @NotNull Registry<T> registry) {
        super(Type.STAND_ALONE, registry, reference.registryKey(), reference.value());
     ...
     */
    public static <T> boolean referencesEqual(RegistryEntry<T> left, RegistryEntry<T> right) {
        if (left == right) {
            return true;
        }
